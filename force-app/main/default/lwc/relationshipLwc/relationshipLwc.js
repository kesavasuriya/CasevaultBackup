...
export default class RelationshipLwc extends UtilityBaseElement {
 ...
 get name() {
    if(this.selectedRow.Name) {
        return 'Showing Relationship for :'+ this.selectedRow.Name;
    }
...
handleRelation() {
    this.loading = true;
    let personList = [];
    getRelationRecord({personId:this.selectedRow.Id, recordId : this.recordId})
    .then(result=>{
        this.loading = false;
        if(result) {
...
handleSave() {
    ...
    console.log('relationshipRecord===',this.relationshipRecord);
