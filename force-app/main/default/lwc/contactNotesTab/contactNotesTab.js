...
const notesColumn = [{ label: 'Contact Date', fieldName: 'Contact_Date__c', type: 'date', typeAttributes: { day: "numeric", month: "numeric", year: "numeric", timeZone: "UTC", tabindex : "-1" } },
...
export default class ContactNotesTab extends NavigationMixin(UtilityBaseElement) {
    ...
    @track collateralList = [];
    @track collateralOptions = [];
    @track changedCollaterals = [];
    @track involvedCollateralValues = [];
    ...
    getNotesInital() {
        ...
        .then(res => {
            ...
            this.collateralOptions = result.involvedCollateralPicklist;
    ...
    handleNotesSubmit(event) {
        ...
        if (this.isValid) {            
            ...
            fields.Contact_Date__c = this.upsertNoteRec.Contact_Date__c;
    ...
    handleNotesSuccess(event) {
       ...
       //if(this.changedPersons.length || this.changedCollaterals.length) {
       ...
       if(this.changedPersons.length) {
            ...
            if(this.changedPersons != null && this.personInvolvedValues != null) {
                ...
                for(let i=0; i < this.changedPersons.length; i++) {
                    ...
                    let foundElement = this.personInvolvedValues.find(ele => ele == this.changedPersons[i]);                    
                    if(!foundElement) {
                        let rec = {};
      ...
      // if(this.changedCollaterals.length) {

        console.log('changedCollaterals===',this.changedCollaterals);
        console.log('involvedCollateralValues===',this.involvedCollateralValues);
            if(this.changedCollaterals != null && this.involvedCollateralValues != null) {

                for(let i=0; i < this.changedCollaterals.length; i++) {

                    let foundElement = this.involvedCollateralValues.find(ele => ele == this.changedCollaterals[i]);                    
                    if(!foundElement) {
                        let rec = {};
                        rec.Notes__c = event.detail.id;
                        rec.Collateral__c = this.changedCollaterals[i];
                        this.createPersons.push(rec);
                    }
                }
                ...
                console.log('changedCollaterals===',this.changedCollaterals);
                console.log('involvedCollateralValues===',this.involvedCollateralValues);
                for(let i=0; i < this.involvedCollateralValues.length; i++) {

                    let foundElement = this.changedCollaterals.find(ele => ele == this.involvedCollateralValues[i]);
                    console.log('foundElement===',foundElement);
                    if(!foundElement) {
                        let rec ={};
                        rec.Id =  (this.collateralList.find(ele => ele.Collateral__c == this.involvedCollateralValues[i])).Id;
       ...
       //}
            ...
            if(this.createPersons.length > 0) {
    ...
    getPersons(id) {
        ...
        .then(res => {
            ...
            for(let i =0; i < response.length; i++) {
                ...
                if(response[i].Person__c) {

                    this.personInvolvedValues.push(response[i].Person__c);
                    this.personInvolvedList.push(response[i]);
                } 

                if(response[i].Collateral__c) {

                    this.involvedCollateralValues.push(response[i].Collateral__c);
                    // let collateralName = '';
                    // if(response[i].Collateral__r.First_Name__c != null) {
                    //     collateralName = collateralName + response[i].Collateral__r.First_Name__c + ' ' ;
                    // }
                    // if(response[i].Collateral__r.Middle_Name__c != null) {
                    //     collateralName = collateralName + response[i].Collateral__r.Middle_Name__c + ' ' ;
                    // }
                    // if(response[i].Collateral__r.Last_Name__c != null) {
                    //     collateralName = collateralName + response[i].Collateral__r.Last_Name__c ;
                    // }
                    // response[i].collateralName = collateralName;
                    this.collateralList.push(response[i]);
                }
    ...
    handleCollateralChange(event) {
        ...
        this.changedCollaterals = event.detail.value;
    }
