...
export default class PersonSearchLWC extends NavigationMixin(UtilityBaseElement) {
    ...
    connectedCallback() {
        ...
        console.log('searchInput===',this.defaultsearchInput);

        if(this.defaultsearchInput && Object.keys(this.defaultsearchInput).length > 0) {

            this.searchInput = {...this.defaultsearchInput};
        }
    ...
    handleSearch() {
        ...
        if(!objHasValue) {
            ...
            getContactDetail({searchJSON:JSON.stringify(this.searchInput), recordId: this.recordId})   
                .then(result => {
                    this.isLoading= false;
                    if (result) {    
    ...
    handleRowSelection(event) {
        ...
        if(selectedPerson) {
            ...
            this.handleEventForSelectPerson(selectedPerson);
    ...
    handleEventForSelectPerson(selectedPerson) {

        const selectPersonEvt = new CustomEvent("selectperson",{
            detail:{personId : selectedPerson.Id}
        });
        this.dispatchEvent(selectPersonEvt);
    }

    handleEventForAddPerson() {

        const addPersonEvt = new CustomEvent("allowaddperson",{
            detail: { disableAddButton : false, searchInput : this.searchInput}
        });
        this.dispatchEvent(addPersonEvt);
    }
