...
deleteRecord(deleteId) {
    ...
    getdifferentElementsFromListofObject(newArray,oldArray) {

    let temparray = [];
    newArray.filter((elem) => {
    let count = 0;
    oldArray.some((ele) => {
    if(ele.childId === elem.childId) count=1;
    });
        if(count === 0) temparray.push(elem);
        });
        return temparray;
    }

    getdifferentElementsFromListofString(newArray,oldArray) {
