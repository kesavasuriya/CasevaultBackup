...
import getRecords from '@salesforce/apex/NCANDSController.getPersons';
import { subscribe, unsubscribe, onError } from 'lightning/empApi';

export default class NcandsLwc extends NavigationMixin(UtilityBaseElement) {
            ...
            .then(result => {
                ...
                for (let i = 0; i < this.personList.length; i++) {
                    ...
                    if (!this.personList[i].Person__r.Date_of_Birth__c) {
                        expression = '<p class="slds-p-around_xx-small" ><b><a href=' + url + '>Child date of birth</a></b></p>';
                    }
                    if (!this.personList[i].Person__r.Gender__c) {
                        expression += '<p class="slds-p-around_xx-small"><b><a href=' + url + '>Child Gender</a></b></p>';
                    }
                    if (!this.personList[i].Person__r.Race__c) {
                        expression += '<p class="slds-p-around_xx-small "><b><a href=' + url + '>Child Race</a></b></p>';
                    }
