...
import getIntakeDetails from '@salesforce/apex/FindingPersonController.getIntakeDetails';
...
export default class UnverifiedPersonLwc extends NavigationMixin(UtilityBaseElement) {
    ...
    handleAddNew() {
        ...
        /*let defaultValue = {};
        if( this.intakePurpose && this.objectApiName == 'Case' ) {
            defaultValue.Intake_Purpose__c = this.intakePurpose;
        }
        defaultValue.Jn_Object_Handler = this.recordId + ',' + this.objectApiName;
        //let Jn_Object_Handler = this.recordId + ',' + this.objectApiName;
                        // if(this.intakePurpose && this.objectApiName == 'Case') {
                //     fields.Intake_Purpose__c = this.intakePurpose;
                // }
        // const encodedValues = encodeDefaultFieldValues({
        //     Jn_Object_Handler__c: Jn_Object_Handler
        // });

        const encodedValues = encodeDefaultFieldValues(defaultValue);
        ...
        if(this.objectApiName == 'Case') {
            getIntakeDetails({ recordId : this.recordId})
            .then(result => {
                
                if( result.Origin == 'Request for services') {
                    this.intakePurpose = result.Service_Program__c;
                } else {
                    this.intakePurpose = result.Origin;
                }

                let Jn_Object_Handler = this.recordId + ',' + this.objectApiName;
                const encodedValues = encodeDefaultFieldValues({
                    Jn_Object_Handler__c: Jn_Object_Handler,
                    Intake_Purpose__c : this.intakePurpose
                });
    ...
    handleSearch(event) {
        ...
        console.log('unVerifiedPersonRecord===',this.unVerifiedPersonRecord);
        this.showModal = true;
    }
    ...
    selectPerson(event) {
        ...
        if(this.objectApiName == 'Case' && this.intakePurpose) {
            fields.Intake_Purpose__c = this.intakePurpose;
        }
    ...
    handleAddNavigate() {
        ...
        if (this.objectApiName == 'Home_Approval__c') {
            ...
            if(this.unVerifiedPersonRecord.Jn_Object_Handler__c) {
                ...
                console.log('intakePurpose===',this.intakePurpose);
