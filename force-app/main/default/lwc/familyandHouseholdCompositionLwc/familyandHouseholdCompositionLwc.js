...
export default class FamilyandHouseholdCompositionLwc extends UtilityBaseElement {
    ...
    handleSave() {
        ...
        this.loading = true;
        console.log('familyRiskReassessmentJSON===',this.assessmentRec);
        if(!this.assessmentRec.Id) {
            this.createList = [];
        }
        console.log('createList===',this.createList);
        console.log('childList===',this.childList);
        savefamilyRiskReassessment({ familyRiskReassessmentJSON : JSON.stringify(this.checkNamespaceApplicable(this.assessmentRec,true)), previousSelectedPersonJSON : JSON.stringify(this.createList), currentSelectedPersonJSON : JSON.stringify(this.childList)})
        .then( result => {
